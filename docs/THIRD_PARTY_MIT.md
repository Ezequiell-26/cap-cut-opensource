...
- RmlUi and OpenCut are tracked as reference projects, not copied into the C++ runtime. RmlUi is useful for UI research but its repository also contains non-MIT third-party assets; OpenCut's current application stack includes TypeScript/Rust, which conflicts with CCOS's C++ runtime rule.
