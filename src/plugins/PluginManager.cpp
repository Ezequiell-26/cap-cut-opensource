...
QVector<PluginDescriptor> PluginManager::scan(const QStringList& directories, QStringList* diagnostics) {
    ...
    for (const auto& directory : directories) {
        ...
        for (const auto& fileInfo : manifests) {
            ...
            const auto o = doc.object(); PluginDescriptor d;
            d.id = o.value(QStringLiteral("id")).toString(); d.name = o.value(QStringLiteral("name")).toString(); d.version = o.value(QStringLiteral("version")).toString(); d.apiVersion = o.value(QStringLiteral("apiVersion")).toString(); d.library = o.value(QStringLiteral("library")).toString();
...
bool PluginManager::isCompatible(const PluginDescriptor& descriptor, const QString& apiVersion) { return descriptor.apiVersion == apiVersion && !descriptor.library.isEmpty(); }
}
