...
QJsonObject encodeTransform(const ccos::timeline::TransformState& t) {
    return QJsonObject{{QStringLiteral("x"), t.x}, {QStringLiteral("y"), t.y}, {QStringLiteral("scaleX"), t.scaleX}, {QStringLiteral("scaleY"), t.scaleY},
                       {QStringLiteral("rotation"), t.rotation}, {QStringLiteral("opacity"), t.opacity}, {QStringLiteral("cropLeft"), t.cropLeft}, {QStringLiteral("cropTop"), t.cropTop},
                       {QStringLiteral("cropRight"), t.cropRight}, {QStringLiteral("cropBottom"), t.cropBottom}, {QStringLiteral("flipHorizontal"), t.flipHorizontal}, {QStringLiteral("flipVertical"), t.flipVertical}};
}
void decodeTransform(const QJsonObject& o, ccos::timeline::TransformState& t) {
    t.x = o.value(QStringLiteral("x")).toDouble(t.x); t.y = o.value(QStringLiteral("y")).toDouble(t.y); t.scaleX = o.value(QStringLiteral("scaleX")).toDouble(t.scaleX); t.scaleY = o.value(QStringLiteral("scaleY")).toDouble(t.scaleY);
    t.rotation = o.value(QStringLiteral("rotation")).toDouble(t.rotation); t.opacity = o.value(QStringLiteral("opacity")).toDouble(t.opacity); t.cropLeft = o.value(QStringLiteral("cropLeft")).toDouble(t.cropLeft);
    t.cropTop = o.value(QStringLiteral("cropTop")).toDouble(t.cropTop); t.cropRight = o.value(QStringLiteral("cropRight")).toDouble(t.cropRight); t.cropBottom = o.value(QStringLiteral("cropBottom")).toDouble(t.cropBottom);
    t.flipHorizontal = o.value(QStringLiteral("flipHorizontal")).toBool(t.flipHorizontal); t.flipVertical = o.value(QStringLiteral("flipVertical")).toBool(t.flipVertical);
}
QJsonObject encodeText(const ccos::text::TextLayer& layer) {
    const auto& s = layer.style();
    return QJsonObject{{QStringLiteral("id"), QString::fromStdString(layer.id().toString())}, {QStringLiteral("text"), layer.text()}, {QStringLiteral("start"), encodeTime(layer.start())},
                       {QStringLiteral("duration"), encodeTime(layer.duration())}, {QStringLiteral("x"), layer.x()}, {QStringLiteral("y"), layer.y()}, {QStringLiteral("family"), s.family},
                       {QStringLiteral("size"), s.size}, {QStringLiteral("color"), s.color}, {QStringLiteral("bold"), s.bold}, {QStringLiteral("italic"), s.italic}, {QStringLiteral("opacity"), s.opacity}};
}
}

bool ProjectSerializer::save(const Project& project, const QString& path, QString* error) {
    QJsonObject root{{QStringLiteral("format"), QStringLiteral("ccos.project")}, {QStringLiteral("version"), 5},
                     {QStringLiteral("id"), QString::fromStdString(project.id().toString())}, {QStringLiteral("name"), project.name()}};
    QJsonArray assets;
    for (const auto& asset : project.assets()) {
        assets.append(QJsonObject{{QStringLiteral("id"), QString::fromStdString(asset.id().toString())}, {QStringLiteral("path"), asset.path()}, {QStringLiteral("name"), asset.name()},
                                  {QStringLiteral("durationMs"), static_cast<qint64>(asset.metadata().durationMs)}, {QStringLiteral("width"), asset.metadata().width}, {QStringLiteral("height"), asset.metadata().height}, {QStringLiteral("fps"), asset.metadata().fps},
                                  {QStringLiteral("videoCodec"), asset.metadata().videoCodec}, {QStringLiteral("audioCodec"), asset.metadata().audioCodec}, {QStringLiteral("audioChannels"), asset.metadata().audioChannels}, {QStringLiteral("sampleRate"), asset.metadata().sampleRate}});
    }
    ...
    for (const auto& track : project.timeline().tracks()) {
        ...
        for (const auto& clip : track.clips()) {
            ...
            clips.append(QJsonObject{{QStringLiteral("id"), QString::fromStdString(clip.id().toString())}, {QStringLiteral("assetId"), QString::fromStdString(clip.assetId().toString())}, {QStringLiteral("start"), encodeTime(clip.start())},
                                     {QStringLiteral("sourceIn"), encodeTime(clip.sourceIn())}, {QStringLiteral("sourceOut"), encodeTime(clip.sourceOut())}, {QStringLiteral("speed"), clip.speed()},
                                     {QStringLiteral("transform"), encodeTransform(clip.transform())}, {QStringLiteral("effects"), effects}});
        }
...
bool ProjectSerializer::load(Project& project, const QString& path, QString* error) {
    ...
    if (version < 1 || version > 5) { if (error) *error = QStringLiteral("Unsupported project version: %1").arg(version); return false; }
    ...
    for (const auto& value : root.value(QStringLiteral("assets")).toArray()) {
        ...
        auto& metadata = asset.metadata(); metadata.durationMs = obj.value(QStringLiteral("durationMs")).toInteger(); metadata.width = obj.value(QStringLiteral("width")).toInt(); metadata.height = obj.value(QStringLiteral("height")).toInt(); metadata.fps = obj.value(QStringLiteral("fps")).toDouble();
        metadata.videoCodec = obj.value(QStringLiteral("videoCodec")).toString(); metadata.audioCodec = obj.value(QStringLiteral("audioCodec")).toString(); metadata.audioChannels = obj.value(QStringLiteral("audioChannels")).toInt(); metadata.sampleRate = obj.value(QStringLiteral("sampleRate")).toInt(); project.addAsset(std::move(asset));
    }
    ...
    if (version >= 4) {
        for (const auto& value : root.value(QStringLiteral("textLayers")).toArray()) {
            const auto o = value.toObject(); ccos::text::TextLayer layer(o.value(QStringLiteral("text")).toString());
            layer.setStart(decodeTime(o.value(QStringLiteral("start")).toObject())); layer.setDuration(decodeTime(o.value(QStringLiteral("duration")).toObject())); layer.setPosition(o.value(QStringLiteral("x")).toDouble(0.5), o.value(QStringLiteral("y")).toDouble(0.85));
