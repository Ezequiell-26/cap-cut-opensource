...
bool TimelineCompositor::build(const ccos::project::Project& project,
    ...
    for (const auto& track : project.timeline().tracks()) {
        for (const auto& clip : track.clips()) {
            ...
            if (track.type() == ccos::timeline::TrackType::Video) {
                ...
                const double cl = std::clamp(t.cropLeft, 0.0, 0.49);
                const double ct = std::clamp(t.cropTop, 0.0, 0.49);
                const double cr = std::clamp(t.cropRight, 0.0, 0.49);
                const double cb = std::clamp(t.cropBottom, 0.0, 0.49);
                ...
                if (cl > 0.0 || ct > 0.0 || cr > 0.0 || cb > 0.0) {
                    vf += QStringLiteral(",crop=iw*%1:ih*%2:iw*%3:ih*%4")
                        .arg(1.0 - cl - cr, 0, 'f', 6).arg(1.0 - ct - cb, 0, 'f', 6).arg(cl, 0, 'f', 6).arg(ct, 0, 'f', 6);
                }
                if (t.flipHorizontal) vf += QStringLiteral(",hflip");
                if (t.flipVertical) vf += QStringLiteral(",vflip");
                vf += QStringLiteral(",scale=iw*%1:ih*%2,rotate=%3*PI/180:fillcolor=black@0")
                    .arg(t.scaleX).arg(t.scaleY).arg(t.rotation);
            ...
            if (asset->metadata().audioChannels > 0 || !asset->metadata().audioCodec.isEmpty()) {
                ...
                af += QStringLiteral(",adelay=%1|%1[a%2]").arg(delay).arg(inputIndex);
    ...
    for (const auto& textLayer : project.textLayers()) {
        ...
        if (expression != current + output) {
            filterComplex += expression + QLatin1Char(';');
            current = QStringLiteral("[%1]").arg(output);
        }
    }
    videoMap = current;
