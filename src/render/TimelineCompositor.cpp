...
QString escapeFilterPath(QString p) { return p.replace('\\', '/').replace(':', '\\:').replace('\'', "\\'"); }
}
...
bool TimelineCompositor::build(const ccos::project::Project& project,
    ...
    for (const auto& track : tracks) {
        for (const auto& clip : track.clips()) {
            ...
            const QString base = QStringLiteral("c%1").arg(inputIndex);
            const QString v = QStringLiteral("v%1").arg(inputIndex);
            const QString a = QStringLiteral("a%1").arg(inputIndex);
            if (track.type() == ccos::timeline::TrackType::Video) {
                ...
                vf += QStringLiteral(",scale=w=min(iw\\,%1):h=min(ih\\,%2):force_original_aspect_ratio=decrease,pad=%1:%2:(ow-iw)/2:(oh-ih)/2")
                ...
                const QString offset = timeText(clip.start());
                vf += QStringLiteral(",setpts=PTS+%1/TB[%2]").arg(offset).arg(v);
                filterComplex += vf + ';';
                videoLabels << '[' + v + ']';
                haveVideo = true;
            }
            ...
            if (asset->metadata().audioChannels > 0 || !asset->metadata().audioCodec.isEmpty()) {
                ...
                af += QStringLiteral(",adelay=%1|%1,volume=1[%2]").arg(QString::number(static_cast<qint64>(clip.start().seconds() * 1000.0))).arg(a);
                filterComplex += af + ';';
                audioLabels << '[' + a + ']';
            }
            ++inputIndex;
    ...
    if (!haveVideo) {
        filterComplex += QStringLiteral("color=c=black:s=%1x%2:r=%3[base];").arg(settings.width).arg(settings.height).arg(settings.fps, 0, 'f', 3);
        videoMap = QStringLiteral("[base]");
    } else {
        ...
        int layer = 0;
        for (const auto& label : videoLabels) {
            const QString out = QStringLiteral("mixv%1").arg(layer++);
            filterComplex += current + label + QStringLiteral("overlay=shortest=0:eof_action=pass:x=0:y=0[%1];").arg(out);
            current = '[' + out + ']';
        }
        videoMap = current;
    }
