...
class DuplicateNodeException : public RenderGraphException {
public:
    explicit DuplicateNodeException(const NodeId& id)
        : RenderGraphException("Node already exists: " + id) {}
};
...
public:
    ...
    std::shared_ptr<T> createNode(Args&&... args) {
        ...
        if (id.empty()) throw InvalidConnectionException("node ID cannot be empty");
        if (nodes_.find(id) != nodes_.end()) throw DuplicateNodeException(id);
    ...
    [[nodiscard]] GraphValidationResult validate() const;
    [[nodiscard]] std::vector<NodeId> getExecutionOrder() const;
    [[nodiscard]] std::vector<NodeId> getRootNodes() const;
    [[nodiscard]] std::vector<NodeId> getLeafNodes() const;

    void setOutputResolution(Resolution res) { outputResolution_ = res; }
    [[nodiscard]] Resolution outputResolution() const { return outputResolution_; }

    void setColorSpace(ColorSpace cs) { colorSpace_ = cs; }
    [[nodiscard]] ColorSpace colorSpace() const { return colorSpace_; }
    ...
    [[nodiscard]] RationalTime duration() const { return duration_; }

private:
    bool hasCycle() const;
    bool hasCycleDFS(const NodeId& node, std::set<NodeId>& visited,
